...
/**
 * Sets the number of products per shop page loop
 *
 * @param  $cols
 *
 * @return  6 or theme option
 */
if (!function_exists('getbowtied_products_per_page')) {
	function getbowtied_products_per_page( $cols ) {
		global $shopkeeper_theme_options;

		return $shopkeeper_theme_options['products_per_page'];
	}

	add_filter( 'loop_shop_per_page', 'getbowtied_products_per_page', 20 );
}
...
if (!function_exists( 'getbowtied_custom_nav_menus' )) {
	function getbowtied_custom_nav_menus() {
		...
		$shopkeeper_theme_options['main_header_off_canvas'] 					= getbowtied_theme_option('main_header_off_canvas', true);
...
/**
 * On theme activation redirect to GetBowtied tools
 */
global $pagenow;

if ( is_admin() && 'themes.php' == $pagenow && isset( $_GET['activated'] ) ) {

	wp_redirect(admin_url("admin.php?page=getbowtied_theme")); // Your admin page URL
...
function redux_to_kirki() {

	if (get_option('shopkeeper_redux_to_kirki') != 1):

	$x = get_option('shopkeeper_theme_options');

	foreach ($x as $k => $v) {
		if (!get_theme_mod($k)) {
			if ( $k == 'main_font' || $k == 'secondary_font' ) {
				$font = str_replace(array('"', "'"), "", $v['font-family']);
				set_theme_mod($k, $font);
			} else if (isset($v['url'])) {
				set_theme_mod($k, $v['url']);
			} else {
				set_theme_mod($k, $v);
			}
		}
	...
	update_option('shopkeeper_redux_to_kirki', 1);
	endif;
}
